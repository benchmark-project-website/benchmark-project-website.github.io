...
export default function LeaderboardSingle({ rows = defaultRows }: Props) {
  ...
  // ✅ 移除 #File / #LOC，仅保留百分比类指标
  const passCols: (keyof Row)[] = ["compile_p1","test_p1","crash_p1","success_p1"];
  const cfCols:   (keyof Row)[] = ["compile_cf","test_cf","crash_cf","success_cf"];
  ...
  const Section: React.FC<{title:string}> = ({title})=>(
    // 1(LLMs) + 4 + 4 = 9 列
    <tr>
      <td colSpan={9} className="bg-gray-50/70 border border-gray-200 px-3 py-2 text-gray-700 italic">
        {title}
      </td>
    </tr>
  );
      ...
      <div className="rounded-xl border border-gray-200 bg-gradient-to-r from-white to-slate-50 p-4 flex flex-col md:flex-row md:items-center md:justify-between gap-3 mb-4 shadow-sm">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            onClick={()=>{
              // ✅ CSV 也去掉 #File / #LOC
              const header = ["Model","Compile%","Test Pass%","Crash%","Success%","Compile%(cf)","Test Pass%(cf)","Crash%(cf)","Success%(cf)"];
              const body = sorted.map(r=>[
                r.model,
                r.compile_p1, r.test_p1, r.crash_p1, r.success_p1,
                r.compile_cf, r.test_cf, r.crash_cf, r.success_cf
              ]);
        ...
        <table className="w-full min-w-[820px] border-separate border-spacing-0">
          <thead>
            <tr>
              ...
              <th colSpan={4} className="sticky top-0 z-10 bg-white border border-gray-200 px-3 py-2 text-center font-semibold">
                Pass@1
              </th>
              <th colSpan={4} className="sticky top-0 z-10 bg-white border border-gray-200 px-3 py-2 text-center font-semibold">
